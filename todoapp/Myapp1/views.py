...
class TaskDetail(LoginRequiredMixin, DetailView):
    model = Task
    context_object_name = 'task'
    template_name = 'Myapp1/task_list.html'
...
class RegisterPage(FormView):
    template_name = 'Myapp1/register.html'
...
class Tasks(LoginRequiredMixin,TemplateView):
    template_name = 'Myapp1/tasks.html'
    def form_valid(self, form):
        form.instance.user = self.request.user
        return super(Tasks, self).form_valid(form)

def log(request):
    return render(request,'Myapp1/logout.html')
